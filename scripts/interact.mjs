...
// --- desktop
{
  ...
  ok(await page.evaluate(() => document.getElementById('hdr-level').textContent === '97' && document.getElementById('s-level').value === '97'), 'level − updates input and header');
...
// --- mobile: settings collapse, nav select, 44px controls
{
  ...
  ok(m1.gridHidden && /Level 98 · max 100 div/.test(m1.sum) && m1.exp === 'false', `mobile settings collapsed with summary "${m1.sum}"`);
...
// --- the Path: first visit lands on it, next step open, tick persists and moves "Next", exchange links hydrate, jump opens
{
  ...
  ok(p1.ticked === 8 && p1.boxes === 25, `${p1.ticked} PoB-verified steps pre-ticked of ${p1.boxes}`);
  ...
  ok(p2.ls && p2.ls.tree === true && p2.next === 'tattoo' && p2.txt === '9' && p2.focus === 'tree', `tick saved (${JSON.stringify(p2.ls)}), Next moved to ${p2.next}, progress ${p2.txt}, focus kept`);
  ...
  ok(p3.ticked === 9 && p3.next === 'tattoo', `tick survives reload (${p3.ticked} ticked, next ${p3.next})`);
