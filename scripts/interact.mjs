...
// --- desktop
{
  ...
  ok(await page.evaluate(() => document.getElementById('hdr-level').textContent === '95' && document.getElementById('s-level').value === '95'), 'level − updates input and header');
...
// --- mobile: settings collapse, nav select, 44px controls
{
  ...
  ok(m1.gridHidden && /Level 96 · max 100 div/.test(m1.sum) && m1.exp === 'false', `mobile settings collapsed with summary "${m1.sum}"`);
